...
public class CreateWarehouseUseCase implements CreateWarehouseOperation {
  ...
  public Either<String, Warehouse> create(Warehouse warehouse) {
    ...
    if (location.maxCapacity <= warehousesPerLocation.stream().map(w -> w.capacity).reduce(0, Integer::sum)) return Either.left(String.format("Location %s has reached max capacity", location.identification));
